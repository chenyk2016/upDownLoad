...
// 个人增加
function UpDownLoad( idName ){
	var _self = this;
	_self.myScroll = new IScroll("#"+idName, {
    ...
    _self.pullDown = {
        ...
        loading:function (d) {
            ...
            _self.addClass(this.el, "loading");
            // 打开上拉功能
            _self.pullUp.locked = false;
        },
        finish: function (fn) {
            
            _self.removeClass(this.el , "loading"  );
	...
	_self.wraper.addEventListener("touchend",function (d) {
...
UpDownLoad.prototype = {
	...
	error:function (text) {
		...
		if (this.prevState === "loadMore" ) {
			this.pullUp.error();
			/*this.noMessage(text || "加载错误");
			var _this = this;
			setTimeout(function (d) {
				_this.closeNoMessage();
			},500);*/
			
		} else{
			
			this.pullDown.error();
			// this.noMessage(text || "刷新失败");
			/*var _this = this;
			setTimeout(function (d) {
				_this.closeNoMessage();
			},500);*/
	...
	//无数据
	/*noMessage: function ( text ) {
		this.wraper.querySelector(".no_mes").innerHTML = text || "暂无数据";
		this.wraper.querySelector(".no_mes").style.display = 'block';
	},
	closeNoMessage:function (text) {
		this.wraper.querySelector(".no_mes").innerHTML = text || "暂无数据";
		this.wraper.querySelector(".no_mes").style.display = 'none';
	},*/
